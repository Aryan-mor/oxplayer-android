# TeleCima — Flutter (Android TV)

This is the Flutter replacement for `tv-app-old` (React). It mirrors the same product shape:

- **Auth gate** (session in `SharedPreferences`, parity with `localStorage`)
...
Optional TV ergonomics: add [`dpad_container`](https://pub.dev/packages/dpad_container) around focusable tiles if you want extra D-pad helpers on top of Flutter’s built-in `Focus` system.
