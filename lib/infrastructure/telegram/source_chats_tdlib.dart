...
/// Lightweight row for the TDLib-backed source picker grid.
...
Future<TdlibPickerChatRow?> tdlibBuildPickerRow({
  ...
  if (t is td.ChatTypePrivate) {
    ...
    return TdlibPickerChatRow(
      chatId: chat.id,
      title: isSelf ? 'Saved Messages' : chat.title.trim(),
      apiChatType: apiType,
      peerIsBot: isBot,
...
/// Best-effort: download small chat photo into [SourcesLocalCache].
Future<String?> tdlibCacheChatPhotoIfNeeded({
  ...
  try {
    ...
    final info =
        await facade.send(td.GetFile(fileId: remote.id)) as td.File;
