...
class _WelcomeScreenState extends ConsumerState<WelcomeScreen> {
  ...
  void initState() {
    ...
    if (!kIsWeb) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        // 2. One more post-frame hack to allow OxplayerApp listener to finish session processing
        await Future<void>.delayed(const Duration(milliseconds: 100));
        auth = ref.read(authNotifierProvider);

        await ref
            .read(appUpdateNotifierProvider.notifier)
            .waitUntilGateReleased();
        if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final showQr = !kIsWeb &&
        ...
        link != null &&
        link.isNotEmpty;
    final showCodeStep = !kIsWeb && pwdStep == null && sms != null;
                  ...
                  children: [
                    ...
                    if (!kIsWeb && showPhoneStep) ...[
                      ...
                      FocusTraversalOrder(
                        ...
                        child: TextField(
                          ...
                          onSubmitted: (_) => unawaited(_onSubmitPhoneNumber()),
                        ),
                      ),
                      const SizedBox(height: 16),
                      FocusTraversalOrder(
                    ...
                    if (!kIsWeb && showCodeStep) ...[
                      ...
                      FocusTraversalOrder(
                        ...
                        child: TextField(
                          ...
                          onSubmitted: (_) => unawaited(_onSubmitLoginCode()),
                        ),
                      ),
                      const SizedBox(height: 16),
                      FocusTraversalOrder(
                    ...
                    if (showQr) ...[
                      ...
                      FocusTraversalOrder(
                        order: const NumericFocusOrder(0),
                        child: OxplayerButton(
                          onPressed: _tdlibBusy
                              ? null
                              : () => unawaited(_onBackFromQrStep()),
                          child: _tdlibBusy
                              ? const SizedBox(
                                  height: 22,
                                  width: 22,
