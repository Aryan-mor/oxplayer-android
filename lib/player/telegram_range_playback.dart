...
class TelegramRangePlayback {
  ...
  Future<Uri?> open({
    ...
    _keyLog('OPEN fileId=$_activeFileId total=${(_activeTotalBytes / (1024 * 1024)).toStringAsFixed(1)}MB');

    try {
      await _download(offset: 0, bytes: _kChunkBytes, synchronous: true)
          .timeout(_kSyncTdlibDownloadTimeout);
    } on TimeoutException {
      _keyLog('OPEN sync bootstrap TIMEOUT ${_kSyncTdlibDownloadTimeout.inSeconds}s → async');
  ...
  Future<Response> _handleRequest(Request request) async {
    ...
    var end = parsed.$2;
    if ((hdr != null &&
            RegExp(r'^bytes=\d+-$').hasMatch(hdr.trim())) ||
  ...
  Future<
    ...
    final syncOk = syncFirstPoke &&
        (lastAvail >= _kMinPrefixBytesForSyncFirst ||
            lastAvail >= need ~/ 2);
    _keyLog(
    ...
    if (_stalePlaybackEpoch(playbackEpoch)) {
      ...
      _keyLog('WAIT ABORT stale epoch=$playbackEpoch now=$_playbackEpoch ms=$ms');
      return (
        ok: false,
        abortedStale: true,
        elapsedMs: ms,
    ...
    if (lastAvail >= need) {
      final ms = DateTime.now().difference(t0).inMilliseconds;
      _keyLog('WAIT END ok=1 ms=$ms repokes=0 avail=${(lastAvail / 1024).toStringAsFixed(0)}KB');
      return (
        ok: true,
        abortedStale: false,
        elapsedMs: ms,
    ...
    while (DateTime.now().isBefore(deadline)) {
      ...
      if (isSeek &&
        ...
        if (facade != null && fid != null) {
          _keyLog('WAIT STALL ${_kSeekStallRecoverAfter.inSeconds}s pre=0 → cancel+repoke');
          try {
            await facade.send(
                td.CancelDownloadFile(fileId: fid, onlyIfPending: false));
          } catch (_) {}
          await _download(
            offset: pokeOffset,
            bytes: _kChunkBytes + _kPrefetchBytes,
  ...
  Future<void> _trimCache(int keepFromOffset) async {
    ...
    _keyLog('TRIM delete+restart from=${(keepFromOffset / (1024 * 1024)).toStringAsFixed(1)}MB '
  ...
  Future<void> _refreshLocal() async {
    ...
    try {
      ...
      if (_activeTotalBytes <= 0) {
        _activeTotalBytes =
            obj.expectedSize > 0 ? obj.expectedSize : obj.size;
      }
    } catch (_) {}
  }
